...
- `TEST_CONTEXT` is set to `Tests detected: yes` plus file list and discovery output, persisted to `.anvil-review-loop/test_context.md` and passed to worker/critic prompts.
...
```bash
cat .anvil-review-loop/test_context.md
# Tests detected: yes
# Test files:
# tests/test_foo.py
